...
ShooterReadings ShooterSubsystem::GetNewReadings() {
  auto left_rpm = 
    converter_.NativeToRealVelocity(left_esc_helper.encoder().GetVelocity())
  ...
  readings.is_ready = 
      units::math::abs(target_speed_ - reading.speed) < 100_rpm &&
...
void ShooterSubsystem::WriteToHardware(ShooterTarget target) {
  ...
  left_esc_helper.Write({frc846::motor::ControlMode::Velocity, native_rpm})
  right_esc_helper.Write({frc846::motor::ControlMode::Velocity, native_rpm})
}
